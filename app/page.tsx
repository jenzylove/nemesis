...
type DiscoveryCandidate={transaction_hash:string;score:number;reasons:string[];goplus_flags:string[];chainabuse_report_count:number|null};
type IncidentDiscovery={source:"bitquery"|"alchemy";selected_transaction_hash:string;selected_score:number;candidate_count:number;incident_time:string|null;candidates:DiscoveryCandidate[]};
type RealResponse={factual_source:"json_rpc";agent_runtime:"google_adk_gemini"|"unavailable";case:{id:string;state:string;wallet_address:string;chain:string;theft_transaction_hash:string|null;discovery:IncidentDiscovery|null;evidence:{transaction:{hash:string;chain:string;block_number:number;timestamp:string;status:"success"|"failed";from_address:string;to_address:string|null;native_value_wei:string;erc20_transfers:Transfer[];nft_transfers:NFTTransfer[]}};finding:{classification:string;summary:string;confidence:number;evidence_references:string[];limitations:string[]}|null;error:string|null}};
